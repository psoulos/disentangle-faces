...
VGG_LATENT_VARIABLE_OP_NAME = 'fc7'
VAE_LATENT_VARIABLE_OP_NAME = 'encoder/means/BiasAdd:0'
...
# Layer Features
vgg_model = VGGFace()
out = vgg_model.get_layer(VGG_LATENT_VARIABLE_OP_NAME).output
...
def encode_img(img_file):
    ...
    vgg_encoding = vgg_model_new.predict(x)

    # VAE encoding
    input_image = process_path(img_file, size=(64, 64))
    # We need to add an extra dimension for a batch size of 1
    input_image = np.expand_dims(input_image, axis=0)
    latent_variable = sess.run(VAE_LATENT_VARIABLE_OP_NAME, feed_dict={'Placeholder:0': input_image})
    # The first index is the batch of size 1 so index into that
    vae_encoding = latent_variable[0]

    return vgg_encoding, vae_encoding
...
for line in open('../stimuli/ImageNames2Celeba.txt', 'r'):
    ...
    vgg_encoding, vae_encoding = encode_img(celeba_file)
    stimulus_to_vae_encoding[stimulus] = vae_encoding
    vgg_encoding = vgg_encoding.squeeze()
    stimulus_to_index[stimulus] = i
    i += 1
    vgg_encodings.append(vgg_encoding)

pca = PCA(n_components=n_components)
vgg_encodings = np.array(vgg_encodings)
if n_components == 4096:
    # Don't perform PCA, just return the default VGG encodings
    pca_encodings = vgg_encodings
    def identity(x):
        return x
    pca.transform = identity
    pca.n_components_ = 4096
else:
    pca_encodings = pca.fit_transform(vgg_encodings)
if args.tag:
    model_name = 'combined.{}.{}.{}.{}'.format(VGG_LATENT_VARIABLE_OP_NAME, VAE_LATENT_VARIABLE_OP_NAME, pca.n_components_, args.tag)
else:
    model_name = 'combined.{}.{}.{}'.format(VGG_LATENT_VARIABLE_OP_NAME, VAE_LATENT_VARIABLE_OP_NAME, pca.n_components_)
...
VAE_LATENT_DIMENSION = 24
LATENT_DIMENSION = pca.n_components_ + VAE_LATENT_DIMENSION
    ...
    for face_run in open(os.path.join(consolidated_subject_bold_dir, FACE_RUNS_FILE_NAME), 'r'):
        ...
        for event_line in open(event_file, 'r'):
            ...
            if base_filename == 'fixation.png':
                ...
                vgg_encoding = pca_encodings[stimulus_to_index[stimulus_filename]]
                vae_encoding = stimulus_to_vae_encoding[stimulus_filename]
                latent_values = np.concatenate((vgg_encoding, vae_encoding))
